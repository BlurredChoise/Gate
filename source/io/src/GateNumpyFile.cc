...
void GateNumpyTree::register_variable(std::string name, const void *p, const size_t size, std::string numpy_format, std::type_index t_index)
{
...
void GateOutputNumpyTreeFile::fill()
{
  ...
  if (!m_vector_of_pointer_to_data.size())
    return;
...
{
  GateFile::open(s.c_str(), std::ofstream::binary | std::fstream::out);
...
void GateOutputNumpyTreeFile::write_variable(std::string name, const void *p, std::type_index t_index)
{
...
//
void GateNumpyTree::register_variable(std::string name, const void *p, std::type_index t_index)
{
...
uint64_t GateNumpyTree::nb_elements() const
{
...
//  cout << "shape_str = #" << shape_str << "#\n";
  ...
  for (auto&& data : v_descr_str)
  {
    ...
    if(  type_str.find("|S") != string::npos  ) // string
    {
        ...
        this->register_variable(name, (const char *)nullptr, number_caracters);
    }
    else
    {
...
void InputNumpyTreeFile::read_entrie()
{

//  cout << "0. current pos = " << m_file.tellg() << " end = " << m_file.end << "eof = " <<  m_file.eof() << "data_to_read() ="  << data_to_read() <<   "\n";
  for (auto&& d : m_vector_of_pointer_to_data) // access by const reference
  {
//    cout << "InputNumpyTreeFile::read_entrie = " << d.m_name << "\ttype = " << d.m_numpy_format<<  "\n";
...
//          char buffer[d.m_nb_characters];
          m_file.read((char*)d.buffer_read,  d.m_size_of_data);
...
void InputNumpyTreeFile::read_variable(const std::string name, void *p, std::type_index t_index)
{
  ...
  for (auto&& d : m_vector_of_pointer_to_data)
  {
    if(name == d.name())
    {
//      cout << "InputNumpyTreeFile::read_variable: found  " << name << "\n";


      if(t_index == typeid(string))
      {
          d.m_type_index_read = t_index;
//          cout << "\tInputNumpyTreeFile::read_variable: string case  " << name << "\n";
      ...
      if(t_index != d.m_type_index)
      {
        ...
        std::stringstream ss;
        if (d.m_type_index != typeid(char*)) // no typeid(char*) in m_tmap_cppToNumpy
      ...
      {
        std::stringstream ss;
//        ss << "object given to store '" << name <<"' is not big enough (given " << this->m_tmapOfSize[t_index] <<" but need at least " << this->m_tmapOfSize[d.m_type_index] << ")";
...
bool GateOutputNumpyTreeFile::s_registered =  GateOutputTreeFileFactory::_register(GateOutputNumpyTreeFile::_get_factory_name(), &GateOutputNumpyTreeFile::_create_method);
